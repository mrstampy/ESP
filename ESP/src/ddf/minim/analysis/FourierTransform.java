 ...
 * <code>timeSize</code> of 1024 and and a <code>sampleRate</code> of 44100 Hz,
 * then the spectrum will contain values for frequencies below 22010 Hz, which
 * is the Nyquist frequency (half the sample rate). If you ask for the value of
 * band number 5, this will correspond to a frequency band centered on
 * <code>5/1024 * 44100 = 0.0048828125 * 44100 = 215 Hz</code>. The width of
 * that frequency band is equal to <code>2/1024</code>, expressed as a fraction
 * of the total bandwidth of the spectrum. The total bandwith of the spectrum is
 * equal to the Nyquist frequency, which in this case is 22050, so the bandwidth
 * is equal to about 50 Hz. It is not necessary for you to remember all of these
 * relationships, though it is good to be aware of them. The function
 * <code>getFreq()</code> allows you to query the spectrum with a frequency in
 * Hz and the function <code>getBandWidth()</code> will return the bandwidth in
 * Hz of each frequency band in the spectrum.
 * <p>
 * <b>Usage</b>
 * <p>
 ...
 * <code>audio</code> is an AudioSource and <code>fft</code> is an FFT (one of
 * the derived classes of FourierTransform).
 * 
 * <pre>
 ...
 * average array every time you use <code>forward()</code>.
 * <p>
...
public abstract class FourierTransform {
   ...
   * referred as a <a
   * href="http://en.wikipedia.org/wiki/Window_function#Rectangular_window"
  ...
  /**
   * A constant indicating a <a href=
   * "http://en.wikipedia.org/wiki/Window_function#http://en.wikipedia.org/wiki/Window_function#Triangular_window_.28non-zero_end-points.29"
   * >Triangular window</a> should be used on sample buffers.
   */
  public static final WindowFunction TRIANGULAR = new TriangularWindow();
  /**
   * A constant indicating a <a href=
   * "http://en.wikipedia.org/wiki/Window_function#Bartlett_window_.28zero_valued_end-points.29"
   * >Bartlett window</a> should be used on sample buffers.
   */
   ...
   * >Bartlett-Hann window</a> should be used on sample buffers.
   */
  ...
  /**
   * A constant indicating a <a
   * href="http://en.wikipedia.org/wiki/Window_function#Blackman_windows"
   * >Blackman window</a> with a default value should be used on sample buffers.
   */
  public static final WindowFunction BLACKMAN = new BlackmanWindow();
  /**
   * A constant indicating a <a
   * href="http://en.wikipedia.org/wiki/Window_function#Gauss_windows">Gauss
   * window</a> with a default value should be used on sample buffers.
   */
   ...
   */
  FourierTransform(int ts, double sr) {
    timeSize = ts;
    sampleRate = (int) sr;
    bandWidth = (2f / timeSize) * ((double) sampleRate / 2f);
    noAverages();
    allocateArrays();
    windowFunction = new RectangularWindow(); // a Rectangular window is
                                              // analogous to using no window.
  }
  ...
  protected void setComplex(double[] r, double[] i) {
    if (real.length != r.length && imag.length != i.length) {
      throw new RuntimeException(
          "FourierTransform.setComplex: the two arrays must be the same length as their member counterparts.");
    } else {
  ...
  protected void fillSpectrum() {
    ...
    if (whichAverage == LINAVG) {
      ...
      for (int i = 0; i < octaves; i++) {
        double lowFreq, hiFreq, freqStep;
        if (i == 0) {
          lowFreq = 0;
        } else {
          lowFreq = (sampleRate / 2) / (double) Math.pow(2, octaves - i);
        }
        hiFreq = (sampleRate / 2) / (double) Math.pow(2, octaves - i - 1);
  ...
  public void linAverages(int numAvg) {
    if (numAvg > spectrum.length / 2) {
      throw new RuntimeException("The number of averages for this transform can be at most " + spectrum.length / 2
          + ".");
    } else {
      averages = new double[numAvg];
    }
    whichAverage = LINAVG;
  }
  ...
  public void logAverages(int minBandwidth, int bandsPerOctave) {
    double nyq = (double) sampleRate / 2f;
    octaves = 1;
    while ((nyq /= 2) > minBandwidth) {
      octaves++;
    }
    avgPerOctave = bandsPerOctave;
    averages = new double[octaves * bandsPerOctave];
    whichAverage = LOGAVG;
  }
  ...
  public void window(WindowFunction windowFunction) {
    if (windowFunction instanceof WindowFunction) {
      ...
      throw new RuntimeException("Invalid window type.");
    }
  }
   ...
   */
  public double getAverageBandWidth(int averageIndex) {
    if (whichAverage == LINAVG) {
      ...
      if (octave == 0) {
        ...
        lowFreq = (sampleRate / 2) / (double) Math.pow(2, octaves - octave);
      }
      // and the high frequency for this octave
      hiFreq = (sampleRate / 2) / (double) Math.pow(2, octaves - octave - 1);
   ...
   *          the new amplitude
   */
  public abstract void setBand(int i, double a);

  /**
   * Scales the amplitude of the <code>i<sup>th</sup></code> frequency band by
   ...
   *          the scaling factor
   */
  public abstract void scaleBand(int i, double s);
   ...
   */
  public int freqToIndex(double freq) {
    // special case: freq is lower than the bandwidth of spectrum[0]
    if (freq < getBandWidth() / 2) return 0;
    // special case: freq is within the bandwidth of spectrum[spectrum.length -
    // 1]
    if (freq > sampleRate / 2 - getBandWidth() / 2) return spectrum.length - 1;
    // all other cases
    double fraction = freq / (double) sampleRate;
  ...
  public double indexToFreq(int i) {
    ...
    if (i == spectrum.length - 1) {
      double lastBinBeginFreq = (sampleRate / 2) - (bw / 2);
      double binHalfWidth = bw * 0.25f;
  ...
  public double getAverageCenterFrequency(int i) {
    if (whichAverage == LINAVG) {
      ...
      double f = lowFreq + offset * freqStep;
   ...
   *          the new amplitude
   */
  public void setFreq(double freq, double a) {
   ...
   *          the scaling factor
   */
  public void scaleFreq(double freq, double s) {
   ...
   */
  public double calcAvg(double lowFreq, double hiFreq) {
   ...
   */
  public abstract void forward(double[] buffer);
   ...
   * @param startAt
   *          the index to start at in the buffer. there must be at least
   *          timeSize() samples between the starting index and the end of the
   *          buffer. If there aren't, an error will be issued and the operation
  ...
  public void forward(double[] buffer, int startAt) {
    ...
    double[] section = new double[timeSize];
   ...
   */
  public abstract void inverse(double[] buffer);
   ...
   */
  public void inverse(double[] freqReal, double[] freqImag, double[] buffer) {
  ...
  public double[] getLogPowerSpectrum() {
    ...
    for (int i = 0; i < spectrum.length; i++) {
      logSpectrum[i] = Math.log10(spectrum[i]);
    }
    return logSpectrum;
  }
}
